...
def DWM_aero(meta,ffor,aero,deficits,turb,inlets_ffor,inlets_ffor_deficits,out,ID_waked):
    ...
    if meta.BEM_AINSLIE_plot:
        ...
        #plt.xlim(1., 0.)
        plt.ylabel('r [R]'), plt.xlabel('U[U0]'), plt.legend()
        plt.show()
        print ' Change this way to plot U_init, put it in the loop specifically for accumulation '
...
def DWM_rotor_aero(meta,aero,ID_waked, *args):
    ...
    if (meta.mean_WS_DWM >= meta.WTG_spec.u_cutin) or (meta.mean_WS_DWM <= meta.WTG_spec.u_cutout) is True:
        ...
        aero.r_t = np.array(BEM.r)/BEM.R    # BEM discretization at rotor plane
...
def DWM_get_deficit(ffor,meta,deficits,ID_waked,inlets_ffor,inlets_ffor_deficits):
    """
    Function calculating the deficits generated by the turbine emitting the wake.
    In the Aero part, you have a number of different approach to manage the accumulation of the deficits:
     - linear
     - quadratic
     - dominant
    :param ffor:
...
def DWM_get_turb(ffor,meta,turb,inlets_ffor_turb,):
    ...
    In the Aero part, the TI accounted is the max from this calculation. there is not an accumulation of TI, but an
    incresing propagation of TI in the wakes.
    Parameters
    ----------
    ffor:(instance of class): Instance of class Ffor holding the fixed frame of reference velocity field in global
    WF coordinates
...
def meand_table_DWM_method(meta):
    ...
    if meta.MEANDERING_plot:
        ...
        plt.xlim(meta.nz-1, 0)
        plt.xlabel('Downstreamwise'), plt.ylabel('Standard Deviation'), plt.legend()
        plt.show()
